...
;( function( $, window, document, undefined ) {
    ...
    $.extend( Plugin.prototype, {
        init: function() {
            ...
            this.maxTransform = $(this.normalCols[0]).height() - $(this.shortCols[0]).height();
            ...
            this._isOnViewport( this.settings.yVisible );
            $window.on( 'scroll resize', this.update.bind( this ) );
        ...
        _setPosition: function( $normalCols, maxTransform, maxScroll ){
            ...
            if( isOnViewport ){
                ...
                if ( Math.abs( transformValue ) < maxTransform ) {

                    $($normalCols).each(function (index, el) {
                        el.style[window._jcsspfx + 'Transform'] = 'translateY(' + transformValue + 'px)';
                    });

                    var height = maxScroll - Math.abs( transformValue )
                    this.$targets[0].style.height =  height + 'px';

                } else if ( Math.abs( transformValue ) > maxTransform ) {

                    transformValue = maxTransform * this.direction;

                    $($normalCols).each(function (index, el) {
                        el.style[window._jcsspfx + 'Transform'] = 'translateY(' + transformValue + 'px)';
                    });
                    this.$targets[0].style.height =  -1 * transformValue + 'px';
                }
                ...
                this.$targets[0].style.height =   maxScroll + 'px';
